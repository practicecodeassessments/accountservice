...
import com.cgi.accountservice.mapper.CustomerMapper;
...
import com.cgi.accountservice.model.AccountResponseModel;
import com.cgi.accountservice.model.CustomerAccountModel;
import com.cgi.accountservice.model.CustomerAccountResponse;
...
public class CustomerService {
    ...
    public ResponseEntity<Object> getCustomerByCustomerId(long customerId){
        ...
        List<AccountResponseModel> accountResponseModelList = new ArrayList<>();
        if(customerOptional.isPresent()){
            NewCustomerModel newCustomerModel = new NewCustomerModel();
            newCustomerModel.setAddress(customerOptional.get().getAddress()  );
            newCustomerModel.setEmail( customerOptional.get().getEmail() );
            newCustomerModel.setFirstName( customerOptional.get().getFirstName() );
            newCustomerModel.setLastName( customerOptional.get().getLastName() );
            customerAccountResponse.setCustomer( newCustomerModel ) ;
            if (!customerOptional.get().getAccounts().isEmpty())
            {
                Optional<Account> account = accountRepository.findByAccountNumber( String.valueOf( customerOptional.get().getAccounts().get( 0 ) ) );
                AccountResponseModel accountResponseModel= new AccountResponseModel();
                accountResponseModel.setAccountNumber( account.get().getAccountNumber() );
                accountResponseModel.setBalance( BigDecimal.valueOf( account.get().getBalance() ) );
                accountResponseModel.setAvailableBalance(BigDecimal.valueOf( account.get().getAvailableBalance() )  );
            }
            customerAccountResponse.setAccountList( Collections.singletonList( accountResponseModelList ) );
        }
        return ResponseEntity.status( HttpStatus.OK)
                .body(customerAccountResponse);*/
        if(customerOptional.isPresent()){
            return ResponseEntity.status( HttpStatus.OK)
                    .body(converter.convertToDto(customerOptional.get()));
        }
            return ResponseEntity.status( HttpStatus.CREATED)
                .body(converter.convertToDto(new Customer()));
    }

    @Transactional(rollbackFor = CreateTransactionException.class)
    public ResponseEntity<Object> createAccount( CustomerAccountModel req) throws CreateTransactionException {
        ...
        return ResponseEntity.status( HttpStatus.CREATED)
                .body(new AccountConverter().convertToDto(createdAccount));
    }

    public ResponseEntity<Object> createCustomer( NewCustomerModel req) throws CreateTransactionException {
        /*Customer customer = CustomerMapper.INSTANCE.DtoToModel(req);
        Customer persistedObject = customerRepository.save(customer);*/
    ...
    public String moneyTransfer( MoneyTransferModel req) throws CreateTransactionException {
        if(req.getDestinationAccountNumber() == null || req.getDestinationAccountNumber().isEmpty()
                || req.getSourceAccountNumber() == null || req.getSourceAccountNumber().isEmpty() ){
        ...
        Account destAccount = accountRepository.findByAccountNumber(req.getDestinationAccountNumber())
                .orElseThrow(()-> new AccountNotFoundException("destination Account not found"));
        if(Math.abs(srcAccount.getAvailableBalance() - req.getMoneyToBeTransferred()) < Double.MIN_NORMAL){
            log.info("Money Can not be debited from source account");
            return " **** MONEY Transfer can not be initiated **** ";
        }
        ...
        return " ####MONEY TRANSFER SUCCESSFULLY COMPLETED ###";
    }
